...
Thank you for considering my application. I look forward to the possibility of discussing how my background, technical skills, and enthusiasm would make me a strong addition to your team.`;
      ...
      return (
        <div className="space-y-4 whitespace-pre-wrap">
          {contentToDisplay.split("\n\n").map((paragraph, index) => (
            <p key={index} className="text-gray-800">{paragraph.trim()}</p>
          ))}
        </div>
      );
    }
  ...
  return (
    <div className="bg-white print-area max-w-4xl mx-auto shadow-lg rounded-lg overflow-hidden">
      ...
      <div className="p-8 font-sans text-base leading-relaxed print:p-0" id="printable-content" ref={letterRef}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 print:grid-cols-2">
          <div className="bg-gray-50 p-4 rounded-lg border-l-4 border-indigo-600">
            <h3 className="font-bold text-indigo-800 mb-2 text-sm uppercase tracking-wider">Applicant Details</h3>
            <p>{address}</p>
            <p>{phone}</p>
          ...
          <div className="bg-gray-50 p-4 rounded-lg border-l-4 border-indigo-600">
            <h3 className="font-bold text-indigo-800 mb-2 text-sm uppercase tracking-wider">Position Details</h3>
            <p>Hiring Manager</p>
            <p className="font-medium">{jobDetails?.position || "Position Title"}</p>
